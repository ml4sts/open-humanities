...
#### File is in .txt or .md or any other plain text format
- Locate the path to the file on from your terminal (you can test that you have the correct path by using `cat` or `head` to view the contents of the file )
- Copy the file to the `my-website` local repo using the `cp` command make sure the copied file is in `.md` 
...
- Use commands like `touch` and/or `nano` to create and fill out the new file with the content that you copied (make sure the new file is `.md`)
- Use `ls` to confirm that the copy was successful
- Use git commands like `add`, `commit` and `push` to push the new change onto your GitHub-based repo
